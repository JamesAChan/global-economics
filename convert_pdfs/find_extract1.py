...
def get_content_table(name_file):
    list1 = []
    list2 = []
    with open('/home/ubuntu/global-economics/contents.json') as data_file:
        data = json.load(data_file)
        listing2 = data[name_file]
        listing2 = [l.strip() for l in listing2]
        print(len(listing2))
        for l in listing2:
            elm = re.sub(r'\s+',' ',l)
            if len(elm.strip()) < 20:
                ...
                elm2 = '^' + elm[0:20].strip()
                list2.append(elm2)
            if 'STAFF APPRAISAL' not in listing2:
                print("Document {} doesn't have Staff Appraisal".format(name_file))
                print("Its last element: {}".format(listing2[-1]))
    #print(list1)
    return list1, list2
...
def get_chunks(newlist, dictionary, headReg, name_file):
    i = 0
    good_list = []
    while i < len(newlist):
        if len(newlist[i])>20:
        ...
        if headReg.match(chn):
            ...
            try:
                ...
                print("IndexError in doc {}".format(name_file))                i = i + 1
        else:
            i = i + 1
...
for f in files_list:
    ...
    # 'tail': everything between the element of content table and next the following element of the content table
        good_list = get_chunks(newlist, dictionary, headReg, name_file)
...
with open('/home/ubuntu/global-economics/file_out2.json', 'w') as outfile:
    json.dump(FinalList, outfile, sort_keys=True,  indent = 4)endTotal=timer()print('Total time is {}'.format(endTotal-startTotal))